...
A closed-book baseline scores 66.8\% answer accuracy vs.\ 88.5\% with retrieval ($+$21.7 pp). Key negative results: BM25+BGE-M3 fusion ($-$27 pp Doc@1), ColSmol visual retrieval (Doc@1 $\approx$ 0), and chain-of-thought prompting (14.3\%, near-random).
\end{abstract}
...
The competition ran on Kaggle (T4$\times$2, 9-hour limit, no internet) and consisted of 15 teams; the winning team scored 0.9598 privately.
...
This paper contributes (1) a complete system description of our v7 pipeline; (2) offline ablations covering retrieval, reranking, and prompting; and (3) a formalized post-competition analysis of risk signals for predicting generalization failure (\S\ref{sec:gap}).
...
Dense retrieval for open-domain QA~\cite{lewis2020retrieval} has been extended to multilingual settings; BGE-M3~\cite{chen2024bge} provides multi-granularity retrieval with strong Ukrainian performance motivating our choice over BM25 or visual methods. Visual retrieval models such as ColPali~\cite{faysse2024colpali} show promise on Latin-script layouts but fail on Cyrillic (\S\ref{sec:results}). Pre-trained models remain brittle under distribution shift~\cite{hendrycks2020pretrained}; our contribution is formalized risk signals (\S\ref{sec:gap}) for predicting generalization failure before submission.
...
MamayLM-Gemma-3-12B-IT~\cite{mamaylm2025}, quantized to Q4\_K\_M (6.8\,GB), is served via \texttt{llama.cpp}~\cite{llamacpp} with greedy decoding ($T=0$). The prompt concatenates the top-2 pages of the chosen document, the question, and six answer options, ending with the directive ``Answer (letter only):'' in Ukrainian; no chat template is applied (an early ablation showed $-$0.028 on fold-1; direct prompting outperforms template-wrapped inference in-distribution). Later reruns on a corrected pipeline version showed positive fold deltas, but these remain untested on Kaggle and are treated as exploratory; v7 retains direct prompting. We apply 3-pass majority voting; unanimous votes (77.2\% of questions) achieve 90.2\% answer accuracy vs.\ 83.7\% for majority-only votes ($+$6.5 pp), confirming voting confidence as a proxy for difficulty.
...
BGE-M3 achieves Doc@1 = 0.933, outperforming BM25 (0.458). Hybrid RRF fusion drops Doc@1 to 0.659 ($-$27.4 pp): BM25's weak Ukrainian performance corrupts BGE-M3's top-1 results. ColSmol-500M~\cite{faysse2024colpali} achieves Doc@1 = 0.039; even after correcting the NaN issue, Doc@1 remains 0.039, consistent with ColPali's training on Latin-script visual layouts rather than Cyrillic text.
...
\caption{Full-dev results ($N$=461). Closed-book: the LLM receives no retrieved context ($k$=0 pages), but document and page predictions still use the dense retrieval top-1 results. Fair 4B vs.\ v7: $\Delta{=}{+}0.023$, bootstrap CI $[{+}0.006,{+}0.041]$. Rerank column shows reranking stage wall time on A30.}
\label{tab:main}
\end{table}
...
\subsection{Prompt and Answering Ablations}
\label{sec:prompt}

Chain-of-thought and elimination prompts collapse to $\approx$14.3\% accuracy (near-random for 6 options), attributable to Q4\_K\_M quantization degrading multi-step reasoning. A Ukrainian instruction prompt also underperforms (58.2\%). Only the direct prompt and an evidence-first variant are effective; direct outperforms evidence-first at pipeline scale (0.867 vs.\ 0.856) because the longer template competes with context space. Sweeping $k$ (100 randomly sampled questions, seed=42): accuracy peaks at $k$=2--3 (92--93\%) and degrades at $k$=5 (89\%); v7 uses $k$=2. LapaLLM on fold 0 achieves composite 0.8430 ($+$0.012) but does not exceed v7's multi-split robustness. Single-pass voting matches 3-pass on fold 0 but is marginally weaker on fold 1 ($-$0.003); three passes remains the default.
...
\node[font=\scriptsize, anchor=south west] at (axis cs:0.8644,0.8698) {v7};
\node[font=\scriptsize, anchor=north] at (axis cs:0.7434,0.6676) {v9};
\node[font=\scriptsize, anchor=north east] at (axis cs:0.8617,0.8211) {v13};
% v14/v11 label offset downward-left with leader line
\node[font=\scriptsize, anchor=north east] (v14label) at (axis cs:0.8750,0.8260) {v14/v11};
\draw[thin, gray] (v14label.east) -- (axis cs:0.8811,0.8320);
% sg label offset upward-right with leader line
\node[font=\scriptsize, anchor=south west] (sglabel) at (axis cs:0.8860,0.8465) {sg};
\draw[thin, gray] (sglabel.south west) -- (axis cs:0.8817,0.8415);
\end{axis}
\end{tikzpicture}
\caption{Local full-dev vs.\ Kaggle public composite. sg\,=\,structure-guard (\S\ref{sec:gap}). Only v7 lies above the $y{=}x$ diagonal; all other systems regressed. Horizontal bars show 95\% bootstrap CI on the local full-dev composite (x-axis) for v13 and v14; v13's interval overlaps the v7 reference (dotted), confirming it was never a decisive offline win.}
\label{fig:gap}
\end{figure}
...
\textbf{v7} & \textbf{Conservative rollback} & \textbf{.8634} & \textbf{.8688} & \textbf{.8722} \\
...
\caption{Risk signals vs.\ Kaggle public outcome. ``Flip\%'' = doc change rate vs.\ v7 (full-dev). ``Imbal.'' = any single per-domain $|\Delta|{>}0.01$ in full-dev composite vs.\ v7. ``Long-doc'' = heuristic targeting docs $\geq$25 pages. All four regressed systems showed $\geq$1 high-risk signal; v7 showed none.}
\label{tab:risk}
\end{table}

Table~\ref{tab:risk} formalizes these signals. A system passes the risk filter only if flip rate $<$5\%, no per-domain delta exceeds 0.01, and no long-document heuristic is active. All four post-v7 Kaggle-tested systems violated at least one criterion; v7 violated none, suggesting this as a practical pre-submission gate for future entries. We note this filter is validated post-hoc on five submissions from a single competition; its generality requires confirmation across other shared tasks and domain-shift scenarios.
...
Page localization difficulty differs sharply: Domain 1 wrong-page cases have \textbf{median distance 7 pages} (within-3-pages: 40\%), while Domain 2 has \textbf{median distance 1 page} (within-3-pages: 76\%). This per-domain asymmetry directly explains why heuristics targeting long-document page retrieval (\S\ref{sec:gap}) showed large Domain 1 gains but failed to generalize.
...
All offline ablations use two visible domains; the hidden third private domain drives the local-vs-Kaggle gap and is not directly analyzable. Q4\_K\_M quantization of MamayLM reduces reasoning coherence — evidenced by CoT collapse to $\approx$14.3\% accuracy, consistent with broader findings that aggressive quantization can reduce reasoning coherence~\cite{dettmers2023qlora}; full-precision inference was infeasible within the 24~GB VRAM budget when co-loaded with the reranker. The 4B reranker, while showing robust offline improvement, could not be submitted within the 9-hour competition deadline on T4$\times$2 hardware.

\section*{Ethics Statement and Reproducibility}

MamayLM and BGE-M3 training data may contain societal biases, and domain-specific biases may affect performance on the hidden domain. All code, pipeline preset files (\texttt{pipeline\_presets.json}: 35 configurations), and per-question predictions for all offline benchmark runs are released at \url{https://github.com/Taleef7/UNLP-2026}. MamayLM and BGE-M3 weights are publicly available. This work complies with the ACL reproducibility checklist.
