...
def extract_answer_traced(text: str):
    """Return (letter, branch). Same decisions as extract_answer -- it just says WHICH rule fired.

    The last branch is an unconditional `return "A"`: when the model emits
    nothing parseable we silently answer A and score it like any other prediction. Zheng et al. (2024)
    show LLMs are already biased toward option "A", which makes our fallback INDISTINGUISHABLE from a
    known model behaviour -- we cannot tell a real A from a parser failure, and neither can a reviewer.

    The fallback leaves a signature and it is present in our own output: on the deterministic greedy run
    "A" is over-predicted (90 predicted vs 79 gold) AND its precision is the lowest of all six letters
    (0.767, against 0.878-0.938 for B-F). Inflated and less precise is exactly what a blind guess does.
    But that is correlational. Tracing the branch turns it into a count.
    """
    raw = text
    text = text.strip()
    for letter in ANSWER_CHOICES:
        if text.upper().startswith(letter):
            return letter, "prefix_latin"
    for cyrillic, latin in UA_TO_LATIN.items():
        if text.startswith(cyrillic):
            return latin, "prefix_cyrillic"       # homoglyph hit: А/В/С/Е emitted instead of A/B/C/E
    match = re.search(r"\b([A-F])\b", text.upper())
    if match:
        return match.group(1), "word_boundary"
    for char in text.upper():
        if char in "ABCDEF":
            return char, "any_char"
    return "A", "FALLBACK_A"                      # <-- nothing parseable. We guessed, and never said so.


def extract_answer(text: str) -> str:
    return extract_answer_traced(text)[0]
...
class PipelineRunner:
    def __init__(
        ...
        # every answer-extraction decision, recorded. Which branch fired, the raw
        # generation, and whether llama.cpp cut it off at the token cap (finish_reason == "length").
        # Without this, a silent `return "A"` is indistinguishable from a confident answer of A.
        self.answer_trace: List[dict] = []
    ...
    def load_llm(self) -> None:
        ...
        # Two of the three MCQ passes sample at temperature=0.5 (see score_mcq), and
        # llama-cpp-python defaults to seed=-1 -- A NEW RANDOM SEED ON EVERY INSTANTIATION. So the
        # shipped system was never reproducible run-to-run, not even by us, and the seed it used was
        # never recorded. Every `seed=42` in scripts/ is an ANALYSIS seed (a bootstrap resample); this
        # is the first EXPERIMENTAL one in the project.
        #
        # Default stays -1 so existing behaviour is unchanged unless a seed is explicitly requested.
        # The resolved value is recorded in the component manifest either way, so a run that did not
        # pin its seed now SAYS SO instead of being silently irreproducible.
        llm_seed = int(self.preset["llm"].get("seed", -1))
        self.component_manifest["llm_seed"] = llm_seed
        self.component_manifest["llm_vote_temp"] = self.preset["llm"].get("vote_temp", 0.5)
        self.component_manifest["llm_reset_kv_cache"] = bool(
            self.preset["llm"].get("reset_kv_cache", False))
        self.log(f"Loading MamayLM GGUF from {gguf_path} (seed={llm_seed})")
    ...
    def score_mcq(self, question_row: dict, context: str, n_passes: int) -> Tuple[str, List[str]]:
        ...
        # These budgets were hardcoded at 3 (completion)
        # and 4 (chat). "Answer: A" is
        # EXACTLY 3 tokens in English under every tokenizer we measured -- and 3 was the cap. The same
        # string is 5 tokens under MamayLM's Ukrainian-adapted vocabulary. A budget that is adequate in
        # English is arithmetically insufficient in Ukrainian, and when it binds, the parser falls back
        # to "A" without telling anyone. Both are now configurable; the DEFAULTS ARE THE OLD CONSTANTS,
        # so behaviour is bit-identical unless a preset asks for otherwise.
        cap_chat = int(self.preset["llm"].get("max_tokens_chat", 4))
        cap_completion = int(self.preset["llm"].get("max_tokens_completion", 3))

        # THE BUG (found 2026-07-14; it also refutes the token-budget hypothesis above).
        #
        # The v4_evidence prompt ends with "Правильний варіант:" -- no trailing space. The model's
        # natural continuation is "\nA". But "\n" is IN THE STOP LIST, so llama.cpp halts at token
        # ZERO, returns the EMPTY STRING with finish_reason="stop", and `extract_answer("")` falls
        # through every branch to its unconditional `return "A"`.
        #
        # Measured on the deterministic greedy run: 36 of 1383 calls return "" -- ALL of them --
        # affecting 13 questions. We answered "A" to all 13. Their gold answers are
        # A,A,B,C,C,D,D,D,E,E,F,F,F. Two were right by luck, and we banked those two as knowledge.
        #
        # This was invisible from the outside for three reasons at once: the model looked like it
        # answered, "A" is a plausible answer, and Zheng et al. (2024) show LLMs are ALREADY biased
        # toward option A -- so our parser failure is camouflaged as a known model behaviour.
        #
        # NOTE the budget was NEVER the problem: truncated == 0 at 3 AND 8 tokens, and at 64 the model
        # finishes its sentence and still emits no letter. the token-budget hypothesis's arithmetic ("Answer: A" is 3 tokens in
        # English, 5 in Ukrainian) is true about the TOKENIZER and does not bind HERE, because the stop
        # sequence fires first. We report that as the negative result it is rather than quietly keeping
        # the tidier story.
        #
        # Default True = the shipped behaviour, bit-for-bit. Set false to let the model speak.
        stop_nl = bool(self.preset["llm"].get("stop_on_newline", True))
        stops_completion = (["\n", "."] if stop_nl else ["."])
        stops_chat = (["\n", ".", " "] if stop_nl else [".", " "])

        # THE SECOND BUG (found 2026-07-14): our three "votes" are not three draws from one
        # distribution, and the cause is a library optimization we never asked for.
        #
        # llama_cpp/llama.py:882 -- inside generate(), even with reset=True:
        #
        #     if reset and self.n_tokens > 0:
        #         longest_prefix = <shared prefix of the cached ids and tokens[:-1]>
        #         if longest_prefix > 0:
        #             reset = False                     # the reset is CANCELLED
        #             tokens = tokens[longest_prefix:]  # evaluate only the suffix
        #             self.n_tokens = longest_prefix    # ...on top of a WARM kv cache
        #
        # score_mcq calls the SAME prompt three times in a row. Call 1 arrives with the PREVIOUS
        # question's prompt in the cache, so it evaluates a long divergent suffix. Calls 2 and 3 find
        # the entire prompt already cached and evaluate exactly ONE token.
        #
        # Why that changes the OUTPUT and not just the speed: GPU kernels pick their parallel reduction
        # strategy from the SHAPE of the work, and floating-point accumulation is non-associative, so a
        # kernel that is not batch-invariant returns different numbers for the same token depending on
        # how much was evaluated alongside it. He & Thinking Machines Lab, "Defeating Nondeterminism in
        # LLM Inference" (2025-09-10) states our exact case: the reduction order "must be identical
        # regardless of whether 0 tokens are in the KV cache (prefill) or 999 tokens are in the KV cache
        # (decoding)", and handling the KV cache separately from the current token values BREAKS batch
        # invariance. Different cache length -> different reduction order -> different logits -> a
        # different greedy token, from a byte-identical prompt. See the nondeterminism literature.
        #
        # It is not a theory. It makes a falsifiable prediction -- calls 2 and 3 share a code path, so
        # they must ALWAYS agree with each other while call 1 may differ -- and that is exactly what the
        # traces show: 0/1844 disagreements between calls 2 and 3 across four cells, and 4/461 questions
        # where call 1 disagrees with both.
        #
        # The damage compounds with the stop-list bug above. On qid 62 the model answers " B" on call 1
        # and "\nB" on calls 2 and 3; with "\n" as a stop token, calls 2 and 3 return "" -> "A", and
        # `Counter(votes).most_common` lets TWO EMPTY STRINGS OUTVOTE THE MODEL'S CORRECT ANSWER 2-1.
        # On qid 408 no empty string is involved at all: the model emits " F" cold and " A" warm, twice,
        # and the vote destroys a correct answer.
        #
        # Default False = the shipped behaviour, bit-for-bit.
        reset_kv = bool(self.preset["llm"].get("reset_kv_cache", False))

        def _trace(letter, branch, text, finish):
            self.answer_trace.append({
                "qid": question_row.get("Question_ID"), "letter": letter, "branch": branch,
                "text": text, "finish_reason": finish,
                # llama.cpp reports finish_reason="length" iff generation was CUT OFF by the cap.
        ...
        if use_chat:
            def _call(temperature: float) -> str:
                ...
                ch = resp["choices"][0]
                letter, branch = extract_answer_traced(ch["message"]["content"])
                _trace(letter, branch, ch["message"]["content"], ch.get("finish_reason"))
                return letter
        else:
            def _call(temperature: float) -> str:
                # reset() sets n_tokens = 0, so the `reset and self.n_tokens > 0` guard at llama.py:883
                # is False and the full prompt is re-evaluated. Every call then sees the same state.
                if reset_kv:
                    self.llm.reset()
                resp = self.llm(raw_prompt, max_tokens=cap_completion, temperature=temperature,
                                stop=stops_completion)
                ch = resp["choices"][0]
                letter, branch = extract_answer_traced(ch["text"])
                _trace(letter, branch, ch["text"], ch.get("finish_reason"))
                return letter
    ...
    def run(self) -> dict:
        ...
        if self.output_dir:
            ...
            write_json(self.output_dir / "answer_trace.json", {
                "max_tokens_completion": int(self.preset["llm"].get("max_tokens_completion", 3)),
                "max_tokens_chat": int(self.preset["llm"].get("max_tokens_chat", 4)),
                "use_chat_template": bool(self.preset["llm"].get("use_chat_template", False)),
                "n_calls": len(self.answer_trace),
                "calls": self.answer_trace,
            })
